...
def build_graph(client, logger=None, max_replan=9, max_steps=15, deadline=None, db_path=None):
    ...
    g.add_conditional_edges("executor", make_router(max_replan, deadline),
                            {"done": END, "replan": "planner", "fail": END})
    return g.compile()


def solve(binary, client=None, logger=None, max_replan=9, max_steps=15, budget=None):
    ...
    app = build_graph(client, logger, max_replan, max_steps, deadline, db_path)
